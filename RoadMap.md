...
2. Конфигурация секретов.
   ...
   - **Статус:** ✅ `OPENAI_API_KEY`, `OPENAI_MODEL` и `OPENAI_PROMPT_VARIABLES` введены вручную через Cloudflare Dashboard (Workers → Settings → Variables → Secrets), значения подтверждены в интерфейсе без логов CLI.
