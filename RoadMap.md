...
**Текущее состояние:** майлстоун М7 возвращён в работу после проваленной продовой проверки `/admin/export` (отсутствует `ADMIN_EXPORT_TOKEN`); требуется восстановить секрет, повторить внешний тест и параллельно расширить фичу экспортов в Telegram с учётом новых требований к админ-доступу.
...
3. Внешняя проверка.
   ...
   - **Статус:** 🚧 Переоткрыто. Офлайн-проверка (`memory-bank/tests/2025-11-05-admin-export-check.md`) подтверждает корректность CSV, однако 2025-11-04 продовый воркер возвращает `404 Not Found` из-за отсутствия секрета `ADMIN_EXPORT_TOKEN` (см. [memory-bank/issues/admin-export-regression-2025-11-04.md](memory-bank/issues/admin-export-regression-2025-11-04.md)). Необходимо восстановить секрет и повторить продовую проверку.
...
5. Шаг 5 — внешняя проверка `/admin/export` на боевом воркере.
  ...
  - **Статус:** 🚧 Провалено 2025-11-04: `/admin/export` на продовом воркере отвечает `404` (см. [memory-bank/issues/admin-export-regression-2025-11-04.md](memory-bank/issues/admin-export-regression-2025-11-04.md)). Требуется восстановить `ADMIN_EXPORT_TOKEN`, подтвердить привязку `DB`, повторить `curl`-проверку и обновить артефакты.
