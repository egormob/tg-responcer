...
4. **Усиление D1-ретраев** — увеличить попытки и бэкофф с джиттером в `runWithRetry`, логировать деградации. _Проверка 4.1: нагрузочный техтест + лог (метрики успешных записей и времени ретраев)._ 
   - [x] **Шаг 4. Усиление D1-ретраев.** `runWithRetry` расширен до 6 попыток с экспоненциальным бэкоффом и джиттером, добавлены логи после ≥3 попыток и фильтр неретраябельных ошибок; юнит-тесты покрывают длинные цепочки ретраев и безретрайные ошибки.
