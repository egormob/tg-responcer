...
### Активная дорожная карта диагностики — 2025-11-16

1. **Фиксация текущей диагностики** — обновить `memory-bank/diagnostics.md`, зафиксировать симптомы, модули и приоритеты. _Проверок перед следующим шагом нет._
...
2. **Ранний запуск тайпинга** — перестроить `DialogEngine.handleMessage`, чтобы `messaging.sendTyping` вызывался до операций D1; добавить тест на медленный storage. _Проверка 2.1: боевой тест + Cloudflare-лог (метрики задержки typing, ретраи D1, отсутствие fallback)._
...
3. **Целостность записи ответов** — откладывать запись `assistant` в D1 до успешной отправки; покрыть тестом с падением `sendText`. _Проверка 3.1: технический мок + боевой диалог (метрики согласованности истории и отсутствия fallback)._ 
...
4. **Усиление D1-ретраев** — увеличить попытки и бэкофф с джиттером в `runWithRetry`, логировать деградации. _Проверка 4.1: нагрузочный техтест + лог (метрики успешных записей и времени ретраев)._
   ...
   - [ ] 🚧 **Шаг 4.4. D1 стресс-ручка и AI queue конфиг.** Повторный стресс-тест должен пройти без `max_retries_exceeded`; проверяем, что воркер читает `AI_QUEUE_CONFIG` из `AI_CONTROL_KV`. _Проверка 4.4: включить `STRESS_TEST_ENABLED=1`, выполнить нагрузку, убедиться, что `/admin/diag?q=ai-queue` отражает значения из KV и в логах отсутствуют `[d1-stress][max_retries_exceeded]`._
     - **Статус 2025-11-17:** stress-run завершился `max_retries_exceeded` (см. `memory-bank/logs/stress-test-2025-11-17-ai-queue.md`), `/admin/diag?q=ai-queue` показывает дефолты `4/64`. Требуются задачи: (а) сверка ключей `AI_CONTROL_KV` ↔ рантайм, (б) стабилизация бэкоффа/параллелизма стресс-ручки перед повтором.
     - **Статус 2025-11-18 (ретест D1 stress smoke + ai-queue snapshot):** нагрузка с `STRESS_TEST_ENABLED=1` прошла без `max_retries_exceeded`, очередь `ai` оставалась в пределах `active≤1/queued=0/dropped=0`, а `/admin/diag?q=ai-queue` по-прежнему показывает дефолты `maxConcurrency=4`, `maxQueue=64`, `avgWaitMs=0`. Логи `wrangler tail` фиксируют `kvConfig:null`, то есть рантайм не читает `AI_QUEUE_CONFIG` из `AI_CONTROL_KV`, и мягкие отказа (`⚠️ → 🔁💬`) вызваны таймаутами OpenAI Responses (`requestTimeoutMs=12000`). Очередь стабильна, но источник лимитов — дефолт окружения; требуется завести задачи на чтение KV или корректировку `requestTimeoutMs/retryMax`.
    - **Статус 2025-11-19 (Variant C — трассировка источников AI-лимитера):** воркер повышает дефолт `requestTimeoutMs` до 18 000 мс и `retryMax` до 3, читает KV/Env с приоритетом KV и логирует `sources.*`. `/admin/diag?q=ai-queue` теперь возвращает `requestTimeoutMs`, `retryMax` и `sources.maxConcurrency|maxQueueSize|requestTimeoutMs|retryMax`, а `getQueueStats` адаптера OpenAI передаёт эти поля всем потребителям. Новый вывод позволяет фиксировать, какие параметры пришли из `AI_QUEUE_CONFIG`, а какие из Env/дефолта, что упрощает ретест Variant C.
5. **Полный экспорт с пагинацией и UTM** — итерировать `x-next-cursor`, собирать полный CSV и предупреждать о лимите строк. _Проверка 5.1: боевой `/export` + ревизия CSV (метрики числа строк и UTM)._
   - [x] **Шаг 5.1. Гарантированная запись UTM при регистрации (2025-11-20).** Ручной прогон `/start src_TEST-CAMPAIGN` → «ping» подтвердил, что `utm_source` записывается в `users` без деградации, повторные сообщения без payload не вызывают `saveUser`, а `knownUsersCache` удерживает исходную метку. Очередь AI (Variant C) читает лимиты из `AI_CONTROL_KV` и переживает таймауты OpenAI через `⚠️ → 🔁💬` без влияния на UTM. Артефакты: [`reports/REPORT-utm-tracking-20251120.md`](reports/REPORT-utm-tracking-20251120.md).
   - [x] **Шаг 5.2. Стабильный `/start` без вызова AI (2025-11-21).** Приветствие переведено в локальную обработку: `/start` всегда выполняет только UTM-парсинг, `saveUser` и обновление `knownUsersCache`, не создавая запросов в OpenAI. Приветствие захардкожено («Привет, &lt;Имя&gt;!» / «Привет!»), fallback «⚠️ → 🔁💬» больше не появляется при первом сообщении. Подробности и артефакты см. в памятке [`memory-bank/operations/start-command.md`](memory-bank/operations/start-command.md) и tail-логе `logs/start-greeting-tail.json`.
   ...
   - 🔜 **Следующий шаг блока 5.** Использовать обновлённый CSV в аналитике и подготовить отчёт по UTM (BI + self-check), затем переходить к разделению лимитов.
6. **Разделение лимитов пользователей и админов** — вернуть «сырые» rate-limit порты админским фичам, оставить toggle для ядра. _Проверка 6.1: интеграционный тест с `LIMITS_ENABLED=0` + боевой спам `/export` (метрики разрешённых/заблокированных запросов)._ 
7. **Троттлинг рассылок** — ограничить пул отправок и ретраи 429 в `minimal-broadcast-service`. _Проверка 7.1: техтест ≥10 получателей + лог (метрики пула, 429, длительности)._ 
8. **Смягчение self-test OpenAI** — вернуть HTTP 200 с `openAiOk=false` и обновить адаптер для диагностического маркера. _Проверка 8.1: запрос `/admin/selftest?token=…` (метрики `openAiOk`, `errors`, статус-код)._
9. **Финальная синхронизация памяток** — выровнять `memory-bank` и RoadMap, зафиксировать статус проверок. _Проверок не требуется._
10. **Продовый soft self-test 2025-11-11** — зафиксировать текущий ответ `/admin/selftest` (200, `openAiOk:false`, причина `missing_diagnostic_marker`), восстановить диагностический маркер Responses и устранить сопутствующие предупреждения (`safe fallback sent`, `failed to update admin export cooldown kv`). _Проверка 10.1: `/admin/selftest` → 200 с `openAiOk:true`, лог safe fallback содержит `reason` только при реальном сбое; `/admin/export` выполняется без предупреждений в логах._
...
**Шаги и проверки:**
1. Команда `/broadcast` с мгновенной отправкой.
   - **Результат:** whitelisted администратор вызывает `/broadcast`, бот запрашивает текст («Введите текст рассылки»), следующее сообщение от администратора (до 4096 символов) немедленно пересылается всем получателям, подключённым к минимальной модели рассылок. HTTP-роуты, очереди и фильтры не используются; права доступа и лимиты Telegram контролируются напрямую в команде.
   - **Проверка:** модульные тесты покрывают сценарии «нет доступа», «пустой текст», «длинный текст >4096 символов» и успешную отправку; ручной прогон подтверждает, что сообщение получает тестовый чат в течение одной сессии команды.
   - **Статус:** ✅ Команда задокументирована и реализована в минимальной конфигурации: мгновенная рассылка без промежуточных заданий, подтверждена вручную в тестовом окружении.
2. Операционная памятка и ограничения.
   - **Результат:** обновлены `memory-bank/operations/broadcast-operations.md`, `README.md` и справка по Telegram-админам (`docs/telegram-admin-commands.md`) с описанием сценария «/broadcast → введите текст → мгновенная отправка», требованием whitelisting и лимитом длины сообщения. Из документов убраны ссылки на очередь, HTTP API и расширенные команды.
   - **Проверка:** ревью документов показывает отсутствие устаревших инструкций; требования к правам администратора и длине сообщения отражены во всех связанных памятках; ссылки ведут только на доступные шаги.
   - **Статус:** ✅ Памятки синхронизированы, переход на минимальную модель рассылки зафиксирован.
   - **Guardrails:** команда `/broadcast` сохраняется только в мгновенном формате; любые попытки вызвать устаревшие подпотоки (`/broadcast queue`, `/broadcast http`, `/admin broadcast queue` и др.) приводят к явному отказу с подсказкой о поддерживаемом сценарии; при срабатывании rate limit администратор всегда получает ответ с указанием задержки.
3. Контур наблюдения за минимальной моделью.
   - **Статус:** 🚧
   - **Результат:** определён базовый чек-лист мониторинга: проверка `wrangler tail` на наличие ошибок доставки, подтверждение наличия whitelisted ID и протоколирование отправок в журнале.
   - **Критерий приёмки:** после завершения шага проверка подтверждает, что каждое использование `/broadcast` фиксируется в журнале, а администратор получает резюме отправки.

**Риски и предохранители:** запрет пустых сообщений; соблюдение лимита 4096 символов; whitelisting администраторов в `ADMIN_TG_IDS`; ручной мониторинг ошибок в `wrangler tail`.
...
4. Контур AI backpressure (вариант A).
   ...
   - **Шаг 4.5 — smoke-прогон с нагрузкой.**
     - **Результат:** сценарий и артефакты хранятся в `memory-bank/logs/ai-queue-smoke-2025-11-16.md` (лог tail + JSON диагностики); повторить перед переходом к шагу 5.
     - **Проверка:** перезапустить скрипт нагрузки и подтвердить, что `droppedSinceBoot` остаётся 0.
     - **Статус:** ⏳ Ожидает повторного прогона перед запуском следующего шага.
   - ⏳ **Шаг 4.x — gate-роллаут AI backpressure.**
     - **Gate 4.x.a — внутренняя синхронизация.** Обновить `RoadMap.md` и `memory-bank/tasks/m8-ai-backpressure.md` до актуальной версии (2025-11-18), подтвердить `npm run check:roadmap`, свериться с отчётом [REPORT-ai-throughput-20251116](reports/REPORT-ai-throughput-20251116.md).
       - **Статус:** ⏳ Требует фиксации свежих документов и успешного `npm run check:roadmap` на рабочей ветке.
     - **Gate 4.x.b — внешние действия и артефакты.** Повторить smoke-прогон очереди, собрать tail-лог и снимок `/admin/diag?q=ai-queue`, синхронизировать их с операционным журналом.
       - **Статус:** ⏳ Артефакты от 16.11.2025 зафиксированы; перед шагом 5 нужна актуализация и подтверждение стабильности.
