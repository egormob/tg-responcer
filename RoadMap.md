...
5. Устранение паузы между индикатором typing и ответом.
   ...
   - **Статус:** ✅ Менеджер `createTypingIndicator` обновлён: `sendTyping` переотправляется с интервалом, пока не завершится обработка сообщения, добавлены автотесты на поддержание индикатора и остановку после ответа (`apps/worker-main/http/__tests__/typing-indicator.test.ts`).
...
3. Настройка и проверка лимитов сообщений на пользователя.
   - **Результат:** параметры суточного лимита (`RATE_LIMIT_DAILY_LIMIT`, `RATE_LIMIT_WINDOW_MS`) документированы; подтверждено, что при превышении лимита бот отправляет уведомление «Дневной лимит ответов исчерпан…». На период тестов планируется повысить лимит и сократить окно по запросу заказчика.
   ...
   - **Статус:** ⏳ Частично подтверждено (ответ с предупреждением приходит); требуется обновить параметры лимита для тестов и задокументировать повторный результат.
