...
3. Аудит операционного контура рассылок.
   - **Статус:** 🚧
   - **Результат:** подтверждено, что после реализации очереди и планировщика рассылки управляются безопасно через HTTP-роуты и Telegram-команды без обхода троттлинга и требований авторизации.
   - **Подпункты:**
     - **HTTP-роут `/admin/broadcast`.**
       - **Результат:** схема аутентификации и авторизации (заголовки `x-admin-token`/`x-admin-actor`, токены `ADMIN_TOKEN`/`ADMIN_BROADCAST_TOKEN`, флаг `BROADCAST_ENABLED`) задокументирована, покрыты сценарии 405/401/403/4xx, повторные запросы к очереди не приводят к дублям.
       - **Проверка:** ревью роутера `apps/worker-main/http/router.ts` и обработчика `features/broadcast/admin-broadcast-route.ts`; модульные тесты подтверждают обработку ошибок, ручной вызов `POST /admin/broadcast` в стейдже с валидным и невалидным токеном.
       - **Критерий приёмки:** чек-лист по HTTP-роуту подписан, в логах фиксируются попытки с неправильными токенами, а очередь создаёт единственный `jobId` на повторный запрос.
     - **Telegram-команды для рассылок.**
       - **Результат:** в обработчике Telegram вебхука закреплены команды `/broadcast`/`/broadcast_status` (или эквивалентные), проверено, что они требуют whitelisting из `ADMIN_TG_IDS` и используют те же флаги/токены, что HTTP-роут.
       - **Проверка:** модульные тесты Telegram-команд покрывают позитивные и негативные сценарии; ручной прогон в тестовом чате подтверждает отказ для не-администраторов и корректную маршрутизацию для whitelisted ID.
       - **Критерий приёмки:** опубликован перечень команд в `memory-bank/operations/`, результаты теста зафиксированы в журнале прогресса.
     - **Управление и мониторинг рассылок.**
       - **Результат:** описан и протестирован набор админ-команд для проверки статуса (`/broadcast_status`, `/broadcast_cancel <jobId>`), сохранены инструкции по KV-флагам, токенам и процедурам остановки/повтора, данные прогресса синхронизируются с `BroadcastProgressStore`.
       - **Проверка:** юнит-тесты для прогресс-хранилища и планировщика подтверждают идемпотентность и корректное снятие блокировок; операционный гайд содержит контрольный список действий при сбое.
       - **Критерий приёмки:** после проверки все команды возвращают ожидаемые ответы, оператор может остановить и возобновить рассылку по инструкции без правок кода.
4. Внешние действия.
  - **Внешние действия:**
...
**Критерии приёмки:** рассылка отправляется, соблюдая троттлинг, при ошибках отдельные пользователи пропускаются без падения ядра; аудит HTTP-роута, Telegram-команд и управления подтверждён чек-листами, инструкции в `memory-bank/operations/` обновлены.
