...
1. Каталог проверок активной связки (ядро ↔ Telegram ↔ Responses).
   ...
   - **Статус:** 🚧 Список сформирован и расширен; адаптер `adapters/openai-responses` переведён на `model`/`prompt`, остаётся сверка остальных модулей и фиксация проверок.
