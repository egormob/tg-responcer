...
2. Реализация `core/DialogEngine.ts` и контрактов.
   ...
   - **Проверка:** юнит-тест `DialogEngine` с моками (`npm run test -- dialog-engine`) покрывает порядок вызовов и обработку ошибок.
3. Определение портов и NOOP-адаптеров.
   - **Результат:** интерфейсы для Messaging/Ai/Storage/RateLimit и заглушки, возвращающие безопасные значения и логирующие предупреждения, чтобы облегчить отладку.
   - **Проверка:** `npm run typecheck` проходит; unit-тесты NOOP подтверждают отсутствие побочных эффектов.
...
5. Guardrails.
   ...
   - **Проверка:** `npm run lint` без ошибок; `npm run validate-guards` отрабатывает без сбоев; визуальная проверка `CODEOWNERS`.
...
1. Telegram-парсер апдейтов и роут вебхука.
   ...
   - **Проверка:** юнит-тесты на разных видах апдейтов (`npm run test -- telegram-webhook`) и проверка, что неверный секрет отвергается.
2. Messaging адаптер.
   ...
   - **Проверка:** мок-тесты адаптера, подтверждающие корректное формирование запросов и разбор `retry_after`.
...
4. Typing-индикация.
   ...
   - **Проверка:** в Telegram клиенте видна индикация typing перед ответом; подтверждение от команды, лог показывает корректные вызовы.
...
1. Адаптер `AiPort` для Responses.
   ...
   - **Проверка:** юнит-тесты с `fetch-mock` проверяют тело запроса, таймаут и обработку ошибок; `npm run test -- openai-adapter`.
...
3. Интеграция с ядром.
   ...
   - **Проверка:** E2E тест через `wrangler dev` с отправкой сообщения; лог содержит requestId и сэмпл-ответ ассистента.
4. Обработка деградаций.
   ...
   - **Проверка:** тест с симулированным `500` (мок) проверяет fallback; команда подтверждает, что Telegram ответ содержит дружелюбное сообщение об ошибке.
...
2. Фича `features/limits`.
   ...
   - **Проверка:** интеграционный тест с имитацией нескольких запросов; `npm run test -- limits` и проверка логов.
3. Конфигурация KV-флага.
   - **Результат:** `KV` ключ `LIMITS_ENABLED` включает/отключает middleware без перезагрузки воркера.
   - **Проверка:** ручной тест: переключить ключ через `wrangler kv:key put`, убедиться, что лимит срабатывает/не срабатывает без redeploy.
...
2. CSV-генератор.
   ...
   - **Проверка:** тест на генерацию >100 записей, проверка памяти (отсутствие утечек) и корректности заголовков.
3. Внешняя проверка.
...
1. Маршрут `/admin/broadcast` и очередь отправок.
   ...
   - **Проверка:** юнит-тесты проверяют валидацию, формирование задания и старт рассылки.
2. Планировщик отправок.
   ...
   - **Проверка:** тесты симулируют массовую рассылку, проверяют обработку `429`, повторов и идемпотентность.
3. Внешние действия.
   - **Внешние действия:**
...
2. Метрики и алерты.
   ...
   - **Проверка:** отчёт команды о появлении метрик в панели; автоматический smoke-тест подтверждает отправку метрик.
