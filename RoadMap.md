...
5. **Полный экспорт с пагинацией и UTM** — итерировать `x-next-cursor`, собирать полный CSV и предупреждать о лимите строк. _Проверка 5.1: боевой `/export` + ревизия CSV (метрики числа строк и UTM)._
   - [x] **External — Pre-Step 5 Load Monitoring.** DIAG-PACKET 2025-11-13 ([лог](memory-bank/logs/diag-packet-2025-11-13-pre-step5-load-monitoring.md)) фиксирует стабильные `/admin/diag?q=ai-queue` до/после нагрузки (`status:"ok"`, `active:0`, `queued:0`, `droppedSinceBoot:0`, `sources.*="default"`, `kvConfig:null`), прохождение `/admin/d1-stress?durationSec=120&concurrency=8` без `max_retries_exceeded` и редкие `[ai][timeout]` от OpenAI Responses с корректным safe fallback; узкое место — латентность внешнего API, а не воркер.
