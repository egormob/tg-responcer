...
8. Шаг 8 — UTM-метки и справочник deeplink.
   - **Результат:** подготовлена справка по deeplink/`startapp` и ограничениям 64 символов (`memory-bank/references/telegram-deeplink-utm.md`), а также обновлена операционная памятка экспорта (`memory-bank/operations/export.md`) с указанием UTM-колонок CSV.
   - **Проверка:** ревью `RoadMap.md` и памяток подтверждает наличие колонок `utm_label`, `utm_source`, `utm_medium`, `utm_campaign`, `utm_content` и чек-листа deeplink; журнал прогресса содержит запись о запуске шага и результатах документирования.
   - **Статус:** ✅ Документация по UTM и экспорту подготовлена, шаг закрыт без запуска автоматических проверок (изменения носят справочный характер).
