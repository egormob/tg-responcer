...
4. **Усиление D1-ретраев** — увеличить попытки и бэкофф с джиттером в `runWithRetry`, логировать деградации. _Проверка 4.1: нагрузочный техтест + лог (метрики успешных записей и времени ретраев)._
   ...
   - [ ] 🚧 **Шаг 4.4. D1 стресс-ручка и AI queue конфиг.** Повторный стресс-тест должен пройти без `max_retries_exceeded`; проверяем, что воркер читает `AI_QUEUE_CONFIG` из `AI_CONTROL_KV`. _Проверка 4.4: включить `STRESS_TEST_ENABLED=1`, выполнить нагрузку, убедиться, что `/admin/diag?q=ai-queue` отражает значения из KV и в логах отсутствуют `[d1-stress][max_retries_exceeded]`._
     - **Текущее состояние (2025-11-17):** stress-run завершился `max_retries_exceeded` (см. `memory-bank/logs/stress-test-2025-11-17-ai-queue.md`), `/admin/diag?q=ai-queue` показывает дефолты `4/64`. Требуются задачи: (а) сверка ключей `AI_CONTROL_KV` ↔ рантайм, (б) стабилизация бэкоффа/параллелизма стресс-ручки перед повтором.
    - **Решение 2025-11-19 (Variant C).** Включаем чтение `AI_QUEUE_CONFIG` из `AI_CONTROL_KV`, фиксируем источник значений в `/admin/diag?q=ai-queue` и одновременно расширяем окно ожидания OpenAI: `AI_TIMEOUT_MS = 18 000 мс`, `AI_RETRY_MAX = 3` (общий бюджет укладывается в 20 с воркера). Ожидаемый эффект — снижение доли `⚠️ → 🔁💬` до <15 % в админских тестах и <10 % для пользователей без наращивания `maxConcurrency`. Мягкие заглушки трактуем как штатные только при `requestId`/`timeout` в логах OpenAI; всплеск без этих признаков — симптом конфигурационной ошибки. Итог фиксируем в stress-отчёте и повторяем D1-прогон уже с KV-конфигом.
