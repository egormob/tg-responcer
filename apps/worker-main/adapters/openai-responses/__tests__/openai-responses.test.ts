...
import type { AiPort, AiQueueConfigSources, ConversationTurn } from '../../../ports';
...
describe('createOpenAIResponsesAdapter', () => {
  ...
  const createAdapter = (
    ...
    createOpenAIResponsesAdapter({
      ...
      runtime: {
        ...
        sources: options.runtime?.sources,
      },
    });
  ...
  it('exposes queue config metadata in diagnostics stats', () => {
    const fetchMock = createFetchMock();
    ...
    expect(adapter.getQueueStats()).toEqual({
      active: 0,
      queued: 0,
      maxConcurrency: 3,
      maxQueue: 12,
