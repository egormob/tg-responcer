...
## Ретест 2025-11-18 — D1 stress smoke + ai-queue snapshot

### Конфигурация
- `STRESS_TEST_ENABLED=1` активирован на рабочем воркере перед запуском `GET /admin/d1-stress`.
- Параллельно выполнялись диалоги от пользователя и администратора (ручной спам).
- `wrangler tail --format=pretty` показывал блоки `[ai][config]` с `maxConcurrency=4`, `maxQueueSize=64`, `kvConfig:null`.

### Наблюдения `/admin/diag?q=ai-queue`
| Время | active | queued | droppedSinceBoot | avgWaitMs | maxConcurrency | maxQueue | Комментарий |
| --- | --- | --- | --- | --- | --- | --- | --- |
| До теста | 0 | 0 | 0 | 0 | 4 | 64 | Значения совпадают с дефолтами окружения, KV не читается. |
| Под нагрузкой | 1 | 0 | 0 | 0 | 4 | 64 | Многократные `[ai][queue_leave] active=1 queued=0 droppedSinceBoot=0`. |
| После теста | 0 | 0 | 0 | 0 | 4 | 64 | Очередь очистилась, `lastDropAt=null`. |

### Поведение в чатах
- Пользователь начинал получать ответы без задержек; при подключении администратора появились мягкие заглушки `Я на секунду отвлекся... Пожалуйста, отправьте сообщение ещё раз 🔁💬` у части админских и единичных пользовательских запросов.
- Заглушки коррелируют с `(warn) [ai][retry] reason: 'OpenAI Responses request timed out'` и `(error) [ai][timeout]`; событий `max_retries_exceeded` и `[ai][queue_drop]` нет.
- После снятия нагрузки бот сам перешёл к полноценным ответам.

### Метрики мягких отказов
- Точная доля не измерялась инструментально; по tail-логу и переписке ~30–40 % админских запросов и ≤20 % пользовательских ответов временно завершались заглушкой.

### Выводы
- Очередь AI и лимитер стабильны: `droppedSinceBoot=0`, `avgWaitMs=0`, `active` не превышает 1.
- Лимиты берутся из дефолтного окружения (`kvConfig:null`), `AI_CONTROL_KV` не применяется.
- Деградация вызвана таймаутами OpenAI Responses (`requestTimeoutMs=12000`) при параллельных запросах; для смягчения нужны либо повышенные лимиты (например, `maxConcurrency=8`, `maxQueue=128`) через KV, либо настройка `requestTimeoutMs`/`retryMax`.
