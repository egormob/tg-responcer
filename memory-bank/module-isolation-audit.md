...
- Минимальная модель задействует только Telegram-команду `/broadcast` с мгновенной отправкой. HTTP-роут (когда включён) прокидывает рассылку в `waitUntil`, не добавляя новые зависимости.
- Устаревшие реализация очереди и scheduler удалены; активный код не тащит фоновые сторы или poller'ы.
...
- При возвращении к расширенной модели убедиться, что возможные новые очереди или HTTP-роуты не ломают текущую стратегию с `waitUntil` и не блокируют минимальную команду.
