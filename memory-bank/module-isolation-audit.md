...
- Минимальная модель задействует только Telegram-команду `/broadcast` с мгновенной отправкой; HTTP-роут и очередь остаются выключенными.
- Заготовки `admin-broadcast-route.ts` и `broadcast-queue.ts` не активны и продолжают быть изолированными от текущего контура.
...
- При возвращении к расширенной модели убедиться, что заготовки очереди и HTTP-роут по-прежнему не влияют на активную минимальную команду.
