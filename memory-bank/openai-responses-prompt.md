...
### Формат `OPENAI_PROMPT_VARIABLES`
- Значение хранится в Cloudflare Workers → Settings → Variables → **Secrets** и передаётся воркеру как JSON-объект.
- Cloudflare поддерживает ввод JSON напрямую, поэтому можно записывать `{ "tone": "neutral" }` без кавычек вокруг всего объекта. Если переменные не требуются, используйте пустой объект `{}`.
- Любое значение, которое не является plain-object (строка, массив, число), приводит к ошибке `OPENAI_PROMPT_VARIABLES must be a JSON object` при старте воркера и фиксируется в логах `[config]`.
- Диагностика `/admin/envz` показывает `openai_prompt_variables: true`, только если секрет присутствует и парсится без ошибок; при отсутствии или некорректном типе self-test сразу сигнализирует.
...
- 2025-11-13: Не удалось подтвердить или обновить `OPENAI_PROMPT_VARIABLES` через Cloudflare Workers Secrets, поскольку среда разработки не имеет сетевого доступа и учётных данных для `tg-responcer`. Воркер не задеплоен, `/admin/envz` не вызывался; требуются действия в боевой инфраструктуре.
