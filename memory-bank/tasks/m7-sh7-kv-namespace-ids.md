...
3. Выполнить `npx wrangler deploy --dry-run` локально или в CI, убедиться, что в секции `Bindings` перечислены оба KV, и деплой проходит без ошибки 10041.
...
5. Обновить `memory-bank/infrastructure.md` и `memory-bank/verification-protocol.md`, добавив правило запрашивать namespace ID перед стартом задач, использующих новые KV. (Правка документации выполнена в текущем коммите — сверить и закрыть пункт после ревью.)
6. После успешного ретеста закрыть задачу и отметить прогресс в RoadMap (М7.Ш7).

## Артефакты
- Логи `wrangler deploy` с успешным перечислением биндингов.
