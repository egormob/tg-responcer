...
| --- | --- | --- | --- |
| `1` | Активен стандартный rate-limit пользователя | Активен `rawRateLimit` (лимитирует `/export`, `/broadcast`, `/admin/*`) | 60 с TTL в KV, уведомление «Экспорт формируется, подождите 60 секунд» + HTTP 200 (чтобы Telegram не ретраил) |
| `0` | Пользовательские лимиты отключены, сообщения проходят без 429 | `rawRateLimit` остаётся строгим, блокирует повторные `/export` | По-прежнему 60 с TTL и уведомление на повторных попытках; HTTP ответ остаётся 200 для защиты от повторной доставки Telegram |
...
- **ПРОВЕРКА 6.1 «Лимиты при отключённом флаге».** В KV установи `LIMITS_ENABLED=0`, прогоняй интеграционный тест (`npx vitest run apps/worker-main/composition/__tests__/compose.test.ts`) и ручной спам `/export` из админ-чата. Метрики: пользовательская ветка (`/start` → сообщение) не получает 429, а `/export` через того же администратора логирует `admin export cooldown active` и присылает уведомление «Экспорт формируется…» на повторных попытках с шагом ≤60 с (HTTP-ответ остаётся 200, Telegram не ретраит). После проверки верни флаг к `1` и задокументируй значения TTL в журнале прогресса.
