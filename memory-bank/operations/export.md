...
- Для статусов `400/403` кеш whitelist-а принудительно инвалидируется, а в KV (`ADMIN_TG_IDS`) пишется запись `admin-error:<userId>` с JSON `{ status, description, at }`. Это помогает увидеть, кто заблокировал бота или попал в неконсистентный чат.
- Диагностический маршрут `/admin/access` теперь возвращает объект `adminErrors`, где ключи — `userId`, а значения повторяют JSON из KV. Используйте его, чтобы понять, когда последний раз получали ошибку и что именно вернул Telegram.
