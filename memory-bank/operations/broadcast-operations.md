...
- Диалог состоит из двух шагов:
  ...
  2. После выбора аудитории бот пишет «Шаг 2. Пришлите текст для N получателей, /cancel для отмены». Только этот текст уходит выбранным адресатам.
- `/cancel` завершает сценарий без отправки. Успешная отправка подтверждается сообщением «✅ Рассылка отправлена!».
...
3. После выбора аудитории бот пишет «Шаг 2. Пришлите текст для N получателей, /cancel для отмены». `/cancel` завершают сценарий без отправки, текст (≤3970 видимых символов — текущий программный лимит) уходит только выбранным получателям.
...
- Получено подтверждение доставки от минимум N тестовых получателей (фактический список из D1).
...
- Протестировать граничные значения: пустой текст, текст из 3970 символов (проходит), текст на 1 символ длиннее лимита (отклоняется с подсказкой).
3. **Мониторинг**
