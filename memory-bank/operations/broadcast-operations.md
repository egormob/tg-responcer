...
- Команда запускается только через `/broadcast` из whitelisted админского аккаунта. `/admin broadcast`, HTTP-роуты и сторонние триггеры отключены.
- UX делится на последовательность «аудитория → шлагбаум текста → подтверждение»:
  1. `/broadcast` → выбор аудитории (`/everybody` или список user_id/username, дубликаты удаляются, `not_found` попадает в резюме).
  2. После подтверждения аудитории бот включает 1000 мс буферизацию текста. Все сообщения, пришедшие в окно, объединяются в один пакет и сравниваются с лимитами.
  3. Сценарий **A** — один chunk в окне: бот показывает предпросмотр, повторяет количество получателей и ждёт явную команду `/send`. После `/send` сообщение уходит через `minimal-broadcast-service`, а результат фиксируется в `broadcast:last`.
  4. Сценарий **B** — больше одного chunk-а: бот отклоняет ввод, отвечает «Эти данные пришли несколькими сообщениями — текст отклонён. Отправьте заново одной фразой или воспользуйтесь /new_text.» и ждёт либо повторного текста, либо команд `/new_text`/`/cancel_broadcast`.
- `/send` — обязательное подтверждение любой версии текста; `/new_text` возвращает к шагу текста без сброса аудитории, а `/cancel_broadcast` мгновенно завершает сценарий и восстанавливает стандартное меню.
...
- Подтверждение шага М8.Ш1 со свежего хвоста: [`memory-bank/logs/diag-2025-11-30-broadcast-webhook.md`](../logs/diag-2025-11-30-broadcast-webhook.md#tail-успешного-webhook-и-broadcast) фиксирует `/broadcast → аудитория → шлагбаум 1000 мс → /send` без регрессий.
- Быстрая памятка по шлагбауму и подтверждению `/send`: [`memory-bank/operations/text-gate-and-send.md`](text-gate-and-send.md).
...
2. После выбора аудитории бот объявляет «Шаг 2» и включает 1000 мс «шлагбаум» текста. Входящие обновления буферизуются: пока окно не закрылось, предпросмотр не показывается.
3. Если пришёл **один** chunk → сценарий A: бот показывает предпросмотр, повторяет `recipients` и предлагает `/send`, `/new_text`, `/cancel_broadcast`. `/send` подтверждает отправку последнего предпросмотра и запускает рассылку через `waitUntil`; после доставки бот присылает «✅ Рассылка отправлена!».
4. Если пришло **несколько** chunk-ов → сценарий B: бот отвечает «Эти данные пришли несколькими сообщениями — текст отклонён…», логирует `text-gate_rejected` и ждёт явный `/new_text` (возврат к шагу 2) или `/cancel_broadcast` (полный выход). До этих команд любые новые сообщения игнорируются.
5. `/new_text` доступна на любом шаге после выбора аудитории и сбрасывает только текстовую часть; `/cancel_broadcast` очищает pending-сессию, вызывает стандартное меню `/admin` и логирует `broadcast_cancelled`.

### 2025-11-30 — подтверждение шага М8.Ш1
Tail [`diag-2025-11-30-broadcast-webhook.md`](../logs/diag-2025-11-30-broadcast-webhook.md#tail-успешного-webhook-и-broadcast) фиксирует полный UX: `/broadcast → /everybody → буферизация 1000 мс → сценарий A с /send`, `broadcast recipients resolved (source=d1, recipients>0)` и `broadcast pool completed`. Это подтверждает, что шлагбаум и команды `/send`/`/new_text`/`/cancel_broadcast` работают штатно.
