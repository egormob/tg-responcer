...
- Команда запускается только через `/broadcast` из whitelisted админского аккаунта. Варианты `/admin broadcast` или вызовы из роутов отключены.
- Диалог состоит из двух шагов:
  1. Администратор отправляет `/broadcast`, бот отвечает «Шаг 1. Пришлите /everybody или список user_id/username…» (дубликаты убираются, `not_found` фиксируется).
  2. После выбора аудитории бот пишет «Шаг 2. Пришлите текст для N получателей…». Только текст, прошедший шлагбаум и подтверждённый `/send`, уходит выбранным адресатам.
- `/cancel_broadcast` завершает сценарий без отправки (аналог прежнего `/cancel`). Успешная отправка подтверждается сообщением «✅ Рассылка отправлена!». 
- Отправка производится в реальном времени; дополнительные очереди и повторные подтверждения отсутствуют.
- Порядок запуска: выполнять после завершённого шага очередей, но до финальной проверки релиза.
- Соблюдаем лимиты Telegram из памятки [`memory-bank/references/telegram-broadcast.md`](../references/telegram-broadcast.md) и фиксируем отправку в операционном журнале (`broadcast_resolve` + `broadcast_summary`, KV `broadcast:last` с TTL 7 дней).
...
2. Бот отвечает подсказкой об аудитории: `/everybody` или список user_id/username через запятую/пробел (дубликаты убираются, `not_found` сохраняется). Пустые сообщения повторно вызывают подсказку.
3. После выбора аудитории бот пишет «Шаг 2. Пришлите текст для N получателей…». Текст (≤3970 видимых символов — текущий программный лимит) уходит только выбранным получателям после прохождения шлагбаума и подтверждения `/send`.
4. Отправка выполняется в фоне (`waitUntil`), бот подтверждает выполнение сообщением «✅ Рассылка отправлена!».
