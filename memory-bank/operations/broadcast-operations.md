...
- Модель рассылки: мгновенное текстовое сообщение без форматирования, вложений, фильтрации и отложенной отправки. Получатели выбираются из D1 (таблица `users`) без env-переменных и сегментации; рассылка уходит всем найденным записям.
...
- Диалог состоит из двух шагов:
  1. Администратор отправляет `/broadcast`.
  2. Бот отвечает подсказкой «Нажмите /cancel если ❌ не хотите отправлять рассылку или пришлите текст».
- Если в ответ приходит `/cancel`, сценарий завершается без отправки. Любой другой текст или эмодзи отправляется каждому подписчику ровно один раз, после чего бот подтверждает операцию сообщением «✅ Рассылка отправлена!» и закрывает сценарий.
- Отправка производится в реальном времени; дополнительные очереди и повторные подтверждения отсутствуют.
- Соблюдаем лимиты Telegram из памятки [`memory-bank/references/telegram-broadcast.md`](../references/telegram-broadcast.md) и фиксируем отправку в операционном журнале.
...
2. Бот отвечает подсказкой «Нажмите /cancel если ❌ не хотите отправлять рассылку или пришлите текст» и ждёт следующее сообщение от того же администратора.
3. Если администратор отправляет `/cancel`, сценарий завершается. Любой другой непустой текст или эмодзи (≤4096 символов) немедленно доставляется всем получателям минимальной модели рассылок. Отправка выполняется в фоне: Worker использует `waitUntil`, чтобы не блокировать HTTP-ответ.
4. После отправки бот подтверждает выполнение сообщением «✅ Рассылка отправлена!». 
