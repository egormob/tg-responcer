...
## Процедуры редактирования и удаления сообщений
1. **Редактирование (`editMessageText`)**
   - Проверить, что от отправки сообщения прошло не более 48 часов. После истечения окна Telegram вернёт ошибку `message can't be edited`.
   - При необходимости продублировать обновление новым сообщением и отметить в журнале причину (истёк лимит редактирования).
2. **Удаление (`deleteMessage`)**
   - Подтвердить, что сообщение было отправлено менее 48 часов назад. По истечении срока Telegram не позволит удалить его для всех адресатов.
   - Если лимит превышен, зафиксировать факт в журнале и согласовать альтернативный сценарий (например, отзыв рассылки отдельным уведомлением).
3. **Справочные материалы**
   - Актуальные ограничения хранятся в памятке [`memory-bank/references/telegram-broadcast.md`](../references/telegram-broadcast.md); при изменениях в Bot API обновить оба документа.
