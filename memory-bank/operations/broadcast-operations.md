...
## Памятка минимальной рассылки
- Модель рассылки: мгновенное plain text сообщение без форматирования, вложений и отложенной отправки. Получатели выбираются только из D1 (`messages`/`users`) по distinct `chat_id`; сегментация, языковые фильтры и env‑переменные (`BROADCAST_RECIPIENTS`) не используются.
- Команда запускается только через `/broadcast` из whitelisted админского аккаунта. Варианты `/admin broadcast` или вызовы из HTTP-роутов отключены.
- Диалог фиксирован в два шага: 
  1. `/broadcast` → выбор аудитории (`/everybody` или список user_id/username, дубликаты удаляются, `not_found` попадает в резюме).
  2. «Шаг 2» → текст рассылки. Сообщение попадает в 1000 мс шлагбаум: бот ждёт, пока Telegram доставит все фрагменты, объединяет короткие и длинные тексты и допускает только одну порцию. Как только администратор подтверждает предпросмотр командой `/send`, отправка запускается и уходит всем выбранным адресатам через `minimal-broadcast-service`.
- Если Telegram прислал более одного сообщения в это окно (часто при копипасте длинных блоков или медленной сети), бот отклоняет ввод, сообщает «Эти данные пришли несколькими сообщениями — текст отклонён. Отправьте заново одной фразой или воспользуйтесь /new_text.» и просит ввести текст повторно.
- `/send` — обязательное подтверждение любой версии текста; `/new_text` возвращает к шагу ввода, а `/cancel_broadcast` мгновенно завершает сценарий и восстанавливает стандартное меню диалога без рассылки.
- Отправка производится в реальном времени; дополнительные очереди и повторные подтверждения отсутствуют. Все события (`broadcast_resolve`, `broadcast_summary`, `text-gate_rejected`) и итог `broadcast:last` (TTL 7 дней) фиксируются в KV/логах.
- Соблюдаем лимиты Telegram из памятки [`memory-bank/references/telegram-broadcast.md`](../references/telegram-broadcast.md) и запускаем рассылку только после завершения очередей/лимитеров предыдущего релиза.
- Актуальный хвост успешного webhook и `/broadcast`: [`memory-bank/logs/diag-2025-11-30-broadcast-webhook.md`](../logs/diag-2025-11-30-broadcast-webhook.md#tail-успешного-webhook-и-broadcast).

### 2025-11-20 — восстановление и проверка после отката
- Код возвращён к коммиту `f0bfa1b02a8b84227ad5d2d21908573334e7763f` (PR #306), где реализованы дебаунс текста, обязательное `/send` и `/cancel_broadcast`.
- Боевой тест подтвердил, что шлагбаум текста и подтверждение `/send` работают как в релизной версии, отдельный `AGENTS.md` добавлен только для обнаружения документации.
- Подтверждённый результат: при попытке отправить текст длиннее лимита Telegram бот выводит единственное предупреждение «Текст не укладывается в лимит…» и остаётся в сценарии без повторного ответа.

### 2025-11-21 — устранено дублирование предупреждения о лимите
- Проверочный сценарий `/broadcast → /everybody → текст > лимита` завершился единичным сообщением «Текст не укладывается в лимит Telegram, выберите: /new_text чтобы отправить другой текст или /cancel_broadcast для отмены.».
- После предупреждения бот ждёт новое сообщение или `/new_text`/`/cancel_broadcast`, рассылка стартует стабильно только после подтверждения `/send`.
...
- Для отправки используется текущий Telegram-адаптер. Команда `/broadcast` запускает мгновенную рассылку и прокидывает отправку в фоновую задачу через `waitUntil`. Очереди, scheduler и прогресс-стор удалены.
- Порты разделены: диалоговый канал (`messaging`) обрабатывает typing и ответы модели с высоким приоритетом, а рассылочный канал (`messagingBroadcast`) работает через отдельный rate limiter из `BROADCAST_MAX_PARALLEL`/`BROADCAST_MAX_RPS`. Если оба порта делят один адаптер, используется приоритетная очередь, чтобы диалоговые сообщения не ждали окончания очереди рассылок.

## Guardrail pause/resume с jobId (М8.J1–М8.J3)
**Цель:** выдерживать рассылку на 100 000+ адресатов при любых рестартах воркера, 429 или краткосрочном OOM без дублей и без запуска второй рассылки параллельно.

- **Единственный активный jobId.** Перед стартом `/broadcast` проверять `broadcast:progress:*` и телеметрию: если есть `status=running/paused`, команда возвращает карточку статуса и предлагает «Продолжить»/«Отменить» вместо запуска нового пула. Новый namespace не создаём: используем существующие `broadcast:pending:*` (диалог) и добавляем `broadcast:progress:<jobId>` для чекпоинтов.
- **Чекпоинты батчей в KV.** После каждой партии отправок `minimal-broadcast-service` пишет в `broadcast:progress:<jobId>` смещение очереди, хеши текста и аудитории, delivered/failed, throttled429, параметры лимитера (`maxRps`, `concurrency`, `rateJitterRatio`), причину остановки и TTL чекпоинта. TTL фиксирован — 24 ч (`ttlSeconds` + `expiresAt`), обновляется при каждой записи. Телеметрию запусков продолжаем писать в существующие ключи `broadcast:telemetry:*`.
- **Аварийная пауза и уведомление.** При `retry_after` выше порога, OOM-сигнале воркера или ручной отмене обновляем чекпоинт `status=paused`, фиксируем `reason`, отправляем админу карточку «❌/⏸ Рассылка приостановлена» с `jobId`, курсором (`offset/total` и remaining), `delivered/failed/throttled429`, TTL чекпоинта и командами «/broadcast_resume <jobId>», `/broadcast_pause <jobId>`, `/broadcast_status <jobId>`, `/broadcast_end <jobId>`, `/cancel_broadcast`. `/admin/diag?q=broadcast` отражает те же поля.
- **Возобновление без дублей.** Команда `/broadcast_resume <jobId>` и кнопка в карточке читают чекпоинт, валидируют хеши текста/аудитории, снижают `concurrency/maxRps` при повторных 429, пропускают уже доставленных (idempotency chatId+threadId) и продолжают с сохранённого смещения до полного охвата. Успешное завершение удаляет `broadcast:progress:<jobId>`, очищает pending-сессию и записывает итог в `broadcast:telemetry:*`.
- **Диагностика и лимиты.** `/admin/diag?q=broadcast` возвращает telemetry + `progress` (active/paused jobId, remaining/total, причина остановки, TTL чекпоинта, команды продолжения/отмены). Глобальный темп остаётся на уровне 28 rps с джиттером, параметры лимитера сохраняются в чекпоинте и читаются перед resume.
- **Watchdog на большие объёмы.** Темп и память ограничены конфигом батча (число получателей и суммарный объём текста). При росте `retry_after` или времени батча воркер автоматически понижает параллелизм, логирует предупреждение и сохраняет новый темп в чекпоинте. Для регрессии — мок-сценарий ≥100k получателей с принудительными 429/ошибками должен завершаться полной доставкой после одного или нескольких resume без дублей.
- **Действие для RoadMap:** после каждого шага (М8.J1–М8.J3) фиксировать статус в разделе М8 RoadMap и обновлять этот раздел памятки, чтобы команда «Следуй дорожной карте…» запускала актуальные задачи jobId.

### Итог М8.J3 (нагрузочная устойчивость)
...
- `/admin/diag?q=broadcast` дополняется карточкой `progress`: активный/paused `jobId`, remaining/total, delivered/failed/throttled429, причина остановки, TTL чекпоинта (`expiresAt`/`ttlSecondsRemaining`) и готовые команды `/broadcast_resume <jobId>`, `/broadcast_pause <jobId>`, `/broadcast_status <jobId>`, `/broadcast_end <jobId>`, `/cancel_broadcast` из KV-чекпоинта.
...
- `BROADCAST_ENABLED` — опциональный флаг. При отсутствии переменной `/broadcast` включён по умолчанию; чтобы отключить команду, явно
  установите значение «0»/`false`.
...
## Шлагбаум текста
**Цель:** исключить мгновенную отправку случайного текста и поймать случаи, когда Telegram разбивает одно сообщение на несколько обновлений.

**Шаг А — текст.** После подсказки «Шаг 2…» администратор отправляет предполагаемый текст рассылки. Обработчик удерживает сообщение в pending-состоянии на 1000 мс и собирает все обновления, пришедшие в этот интервал. Если Telegram передал более одного фрагмента (`Telegram → несколько сообщений ⇒ отказ`), бот отвечает ошибкой «Эти данные пришли несколькими сообщениями — текст отклонён. Отправьте заново одной фразой или воспользуйтесь /new_text.» и возвращает пользователя к началу шага А.

**Шаг Б — подтверждение.** Если в 1000 мс окне пришёл единственный текст, бот показывает предпросмотр и просит подтвердить командой `/send`. Только после получения `/send` сообщение уходит получателям. Дополнительные команды:

- `/new_text` — отменяет текущий предпросмотр и переводит оператора обратно к шагу А без выхода из сценария.
- `/cancel_broadcast` — мгновенно завершает сценарий с очисткой pending-записи и без отправки сообщений (аналог старого `/cancel`).

**Правило:** пока администратор не отправил `/send`, рассылка считается незавершённой. `/send` всегда подтверждает последнюю версию текста, собранную в шаге А, и сбрасывается после каждой отправки. При повторном `/broadcast` шлагбаум активируется заново.

### Чек-лист приёмки запуска
- В tail есть `broadcast recipients resolved` с `source=d1` и `recipients>0` (проверяем, что выборка из D1 заполнена).
- Получено подтверждение доставки от минимум N тестовых получателей (фактический список из D1) **после** того, как whitelisted оператор отправил `/send`.
- KV-лог pending-сессии очищен после завершения (нет зависших ключей `broadcast:pending:*`).
- Отдельно зафиксирован прогон текстового шлагбаума: бот запрашивает `/send`, а при имитации двух сообщений в 1000 мс отвечает ошибкой «Эти данные пришли несколькими сообщениями — текст отклонён…» и возвращает к шагу А.
...
#### Глобальный лимитер рассылки
- Активен общий rate limiter на 28 RPS с джиттером, управляющий отправкой на уровне пула (`BROADCAST_MAX_RPS` = 28 по умолчанию, джиттер конфигурируется через `BROADCAST_RATE_JITTER_RATIO`).
- Лимитер включён в прод-режиме и должен применяться ко всем режимам рассылки; новые фичи/апгрейды обязаны либо использовать текущую настройку, либо явно документировать её переопределение.
- При изменении параметров лимитера фиксируйте ссылки на релевантные переменные окружения и обновляйте этот раздел, чтобы downstream‑фичи не обходили глобальный контроль скорости.
- `/admin/diag?q=broadcast` возвращает снимок KV-телеметрии лимитера и пула (в том числе `maxRps:28`, `rateJitterRatio`, `poolSize`, `maxAttempts`). Если значения отличаются от зашитых по умолчанию, исправьте конфиг и обновите памятку.

### Чекпоинты и блокировка параллельных рассылок
- Прогресс рассылки сохраняется в KV-чекпоинтах: пул фиксирует метрики `delivered/failed/throttled429`, последний курсор аудитории и `jobId` активной отправки, чтобы при рестарте воркер мог возобновить тот же поток без потери адресатов.
- Если `jobId` активен, новый `/broadcast` возвращает отказ на запуск параллельной рассылки. Это предотвращает гонки между операторами и гарантирует, что один поток завершится или будет возобновлён прежде, чем стартует следующий.
- Диагностика `/admin/diag?q=broadcast` показывает статус чекпоинта (`status`, `cursor`, `jobId`, `remaining/total`, `reason`, TTL), чтобы оператор мог увидеть, на каком шаге остановилась отправка.
- Механизм `pause/resume` активен: при паузе в админ-чате отображается карточка «Рассылка приостановлена» с `jobId`, курсором, `delivered/failed/throttled429`, remaining и TTL чекпоинта; возобновление происходит командой `/broadcast_resume <jobId>` без пересборки аудитории, отмена — `/cancel_broadcast`.
...
- Проблема: при пересоздании воркера между `/broadcast` и текстом рассылки сессия сбрасывалась, и сообщение улетало в основной диалог (админ видел typing/ответ модели, но рассылка не уходила).
- Решение: `telegram-broadcast-command` хранит ожидающий текст не в локальном `Map`, а в общем хранилище, привязанном к окружению воркера. Даже если `routerCache` сброшен, новый обработчик получает тот же `pendingStore` и продолжает сценарий.
- Тест-покрытие: `apps/worker-main/features/broadcast/__tests__/telegram-broadcast-command.test.ts` содержит кейс `keeps pending broadcast after router cache reset`, который очищает `routerCache` между запросами и подтверждает отсутствие вызова диалогового движка.
- Операционная заметка: если админ снова видит ответ модели вместо «✅ Рассылка отправлена!», проверяем, что worker не перезапускался более 60 секунд — TTL pending-сессии прежний (`pendingTtlMs=60000`).
...
2. **Проверка сценария**
   - От whitelisted аккаунта выполнить `/broadcast` → отправить текст (например, «ping») → подтвердить `/send`. Подтвердить получение сообщения на тестовых чатах и убедиться, что без `/send` рассылка не стартует.
   - Попробовать команду с не-whitelisted аккаунта и убедиться, что бот корректно отказывает без отправки.
- Протестировать граничные значения: пустой текст, текст из 3970 символов (проходит), текст на 1 символ длиннее лимита (отклоняется с подсказкой), попытка отправить два сообщения подряд в окно 1000 мс (бот отвечает ошибкой «Эти данные…» и просит /new_text).
3. **Мониторинг**
   ...
   - При каждом запуске фиксировать, что сначала пришёл предпросмотр текста и запрос `/send`, а подтверждение появилось в tail (событие `broadcast send confirmed`).
   ...
   - Проверить `GET /admin/diag?q=broadcast` — последний запуск должен отображаться с актуальными `delivered/failed/throttled429/durationMs`
     и статусом `ok` (или `aborted`, если сработала защита). Ошибка «Эти данные…» должна логироваться как `text-gate_rejected`, чтобы её можно было найти при ретроспективе.
...
- Telegram-команда `/broadcast` использует sender рассылки напрямую. Обработчик сразу отвечает клиенту (`202 Accepted`) и регистрирует отправку через `waitUntil`, чтобы сообщения ушли даже после закрытия запроса.
...
### Спроектировано
- Источник получателей: выборка D1 (`users`) → fallback none.
...
- Логи `broadcast recipients resolved` и `broadcast pool initialized` пишутся с `source=d1` и заполненным списком (`recipients>0`).
- Пул sender’ов отрабатывает без ошибок и фиксирует `broadcast pool completed`/`admin broadcast delivered` с ненулевой доставкой; Telegram-статус подтверждает ответ администратору и получателям.
- Свежие хвосты webhook и `/broadcast`: [`memory-bank/logs/diag-2025-11-30-broadcast-webhook.md`](../logs/diag-2025-11-30-broadcast-webhook.md#tail-успешного-webhook-и-broadcast) фиксируют успешную доставку по D1.
