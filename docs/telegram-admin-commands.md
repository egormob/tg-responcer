...
4. **Шаг А — текст:** после выбора аудитории бот просит прислать текст (до 3970 символов). Сообщение попадает в 1000 мс «шлагбаум»: если Telegram доставил более одного обновления, бот отвечает «Эти данные пришли несколькими сообщениями — текст отклонён. Отправьте заново одной фразой или воспользуйтесь /new_text.» и возвращает к началу шага А.
5. **Шаг Б — подтверждение:** при одиночном сообщении бот показывает предпросмотр и ждёт `/send`. Только после `/send` текст отправляется получателям. Дополнительные команды: `/new_text` (перезаписать текст), `/cancel_broadcast` (полностью отменить сценарий). Без `/send` рассылка не стартует.
