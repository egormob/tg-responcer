...
4. После выбора аудитории бот пишет «Шаг 2. Пришлите текст для N получателей…»: отправьте текст (до 4090 символов), `/cancel` отменяет сценарий.
5. По завершении бот отправляет подтверждение «✅ Рассылка отправлена!» и записывает `broadcast:last` в `ADMIN_EXPORT_LOG` (TTL 7 дней) с фактическими метриками доставки, сэмплом чатов и списком `not_found`.
