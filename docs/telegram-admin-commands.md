...
2. Бот ответит подсказкой «Пришлите текст для N получателей и подтвердите отправкой /send или сообщением. Нажмите /cancel если ❌ не хотите отправлять рассылку.».
3. Источник получателей — только D1 (данные чатов из таблиц `messages`/`users`), сегментация и фильтрация по языкам отключены.
4. Если администратор вводит `/cancel`, запуск команды завершается без отправки.
5. После подсказки требуется явное подтверждение: отправьте текст рассылки (или `/send <текст>`) — только это сообщение попадёт в доставку.
6. По завершении бот отправляет подтверждение «✅ Рассылка отправлена!» и записывает `broadcast:last` в `ADMIN_EXPORT_LOG` (TTL 7 дней) с фактическими метриками доставки и сэмплом получателей.
