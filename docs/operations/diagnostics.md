...
## Диагностическая карточка: AI Queue Stress Test (после фикса сериализации)

> Стресс-прогон очереди AI после миграции payloadов на `input_text`.

### 1. Контекст проверки

- Проверялось применение `AI_QUEUE_CONFIG` из KV в рантайме после фикса сериализации system-turn.
- Цель — убедиться, что конфигурация лимитера (конкурентность, очередь, таймауты и ретраи) выдерживает пиковую нагрузку без failover.

### 2. Фактический результат stress-run

- Источники: Cloudflare tail (`ai-queue-stress-tail.json`) и `/admin/diag?q=ai-queue`.

#### 2.1. Состояние очереди

- `status:"ok"`, `active:0`, `queued:0`, `droppedSinceBoot:0`, `avgWaitMs:0`, `lastDropAt:null`.
- Лимиты читаются из KV: `maxConcurrency:4`, `maxQueue:64`, `retryMax:3`, `requestTimeoutMs:18000`.
- `sources.*="kv"` подтверждает загрузку конфигурации из `AI_QUEUE_CONFIG`.

#### 2.2. Эндпоинты и failover

- `activeBaseUrl=https://api.openai.com/v1/responses`.
- `backupBaseUrls=["https://api.openai.com/v1/responses?cf_region=eu"]`.
- `failoverCounts.endpoint_1=0`, `failoverCounts.endpoint_2=0`; в логах нет переключений, failover не активировался.

#### 2.3. Работа адаптера OpenAI под нагрузкой

- Для каждого запроса виден конвейер `[ai][request] → [ai][ok] → [ai][parsed]`.
- Ошибок класса `AI_NON_2XX` нет.
- Таймауты (`[ai][timeout]`) отрабатываются через ретраи (`[ai][retry]`), после чего запросы завершаются успехом.

#### 2.4. D1-stress сигналы

- Tail фиксирует `Too many API requests by single worker invocation`, помеченные как `retryable:true` без `max_retries_exceeded`.
- Поведение соответствует адаптивному троттлингу.

### 3. Выводы по стабильности

- Очередь выдерживает нагрузку: дропов нет, failover не срабатывает, конфигурация корректно читается из KV.
- OpenAI-интеграция стабильна, ретраи закрывают кратковременные таймауты.
- Прогон соответствует критериям RoadMap 4.4.

### 4. Сигналы для документации и улучшений

1. **Таймауты и ретраи.** Краткие `[ai][timeout] → [ai][retry]` ожидаемы под нагрузкой; при масштабировании стоит задокументировать их допустимую долю.
2. **Failover-диагностика.** Полезно расширить `/admin/diag?q=ai-queue` полями `activeEndpointId` и `failoverCounts`, чтобы быстрее понимать деградацию конкретного base URL.
3. **Валидация AI_BASE_URLS.** Рассмотреть whitelist допустимых хостов или валидацию KV, чтобы исключить ошибочные URL.

### 5. Ответы на операционные вопросы

1. **Статус системы.** Все проверки прошли успешно; очередь и адаптер работают штатно, критичных доработок не требуется.
2. **Пропускная способность.** `maxConcurrency:4` при `avgWaitMs:0` даёт устойчивую обработку ≈4 сообщений в секунду (по одному на поток) при средних 1-секундных инференсах; запас по очереди `maxQueue:64` покрывает кратковременные пики.
3. **Поведение при превышении нагрузки.** Новые запросы будут накапливаться в очереди до 64 штук; если лимит превышен, воркер вернёт дроп (`droppedSinceBoot` начнёт расти), сохраняя приоритет уже взятых задач.
4. **Замедление OpenAI.** Клиент увидит дольше тянущийся `sendTyping`, пока воркер ждёт до `requestTimeoutMs` (18 секунд). При устойчивой задержке поверх таймаута сработает ретрай до `retryMax:3`; только после полного истечения попыток пользователь получит сообщение об ошибке вместо typing-индикации.
...
> Используй ручку после ручного теста `/start src_TEST-CAMPAIGN` → «ping»: пока кэш не очищен, ответ покажет `size` и `userIdHashes[]` для всех пользователей с зафиксированным `utmSource`. Это позволяет подтвердить, что цепочка `/start` → `knownUsersCache` → повторное сообщение действительно прокинула UTM и что значение будет доступно экспорту даже без повторного payload.
