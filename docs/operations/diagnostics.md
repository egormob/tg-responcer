...
### Чеклист приёмки `/start <utm>`
1. Отправь пользователю диплинк `/start <utm>` и зафиксируй цепочку логов `[utm-tracking] incoming payload` → `[telegram-webhook] normalized message` → `[utm-tracking] saveUser result … utmSource:"<utm>" utmDegraded:false`.
2. Выполни `wrangler d1 execute DB --command "SELECT user_id, utm_source FROM users WHERE user_id='<id>'"` и проверь, что `utm_source` равен исходной метке.
3. Отправь 1–2 сообщения без payload и убедись по tail-логу, что `hasStartPayload:false` и повторных `saveUser result` для того же `userId` нет — это подтверждает работу `knownUsersCache`.
4. При необходимости вызови `/admin/diag?q=ai-queue`, чтобы исключить деградацию очереди/таймауты OpenAI как первопричину fallback «Я на секунду отвлекся... Пожалуйста, отправьте сообщение ещё раз 🔁💬».
...
## Диагностическая карточка: Экспорт (Шаг 5.3) — Полнота экспорта

> Боевой прогон `/export` в админ-чате. Проверка синхронизирована с шагом М7.Ш5.3 RoadMap.

### 1. Итог проверки

- Получено 1 754 строки — больше лимита 1 000 строк на страницу, следовательно, пагинация D1 задействована.
- Курсор `x-next-cursor` обработан ≥2 раза, CSV собран из всех страниц без пропусков или дублирования заголовков.
- Заголовок экспорта содержит валидные UTM-метки (`src_DIAG`, `src_TEST-GREETING`, `stress_test`), в CSV у каждой строки заполнен `utm_source`.
- Экспорт завершился без ошибок `empty export`, `truncated`, `[d1-storage] column missing` и т. п.
- Бот успешно доставил CSV в Telegram-чат; файл открывается в Numbers/Excel без предупреждений.
...
Ошибок `d1-storage` в логах нет.

### 3. Структура CSV

- Колонки: `id, tg_user_id, tg_username, tg_first_name, tg_last_name, last_message_at, utm_source, created_at`.
- Количество строк: 1 754.
- Выборочный контроль значений `utm_source` подтверждает соответствие заголовку экспорта.

### 4. Побочное наблюдение и мера

- В tail зафиксировано `KV PUT failed: 400 Invalid expiration_ttl of 30` — Cloudflare KV требует TTL ≥ 60 секунд для кулдауна `/export`.
- TTL повышен до 60 секунд (см. `apps/worker-main/features/export/telegram-export-command.ts`), чтобы rate-limit больше не пропускал повторы и не логировал предупреждение.

### 5. Критерии повторной проверки

1. Вызвать боевой `/export` по диапазону с ≥1 500 строк; убедиться, что `rowCount` в логах > 1 000 и есть хотя бы один `x-next-cursor`.
2. Проверить, что итоговый CSV содержит ожидаемые колонки и полностью собирается (нет дублирования заголовка между страницами).
3. Сверить `utmSources` из логов с содержимым колонки `utm_source` в нескольких выборочных строках.
