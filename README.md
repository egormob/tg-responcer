...
Перед запуском `/broadcast` укажите переменную окружения `BROADCAST_RECIPIENTS`:

- поддерживаются JSON-массивы строк (`["123","456"]`) и объектов (`[{"chatId":"123","username":"alice"}]`);
- можно передать простую строку со списком через запятую/точку с запятой/перевод строки (`"123,456\n789"`);
- для сопоставления username и chat_id используйте разделители `=` или `:` (`"@alice=123456"`);
- поля `username` и `threadId` опциональны и пригодятся для будущей фильтрации рассылок по списку пользователей.

Пустое значение приводит к предупреждению в логах (`[broadcast] BROADCAST_RECIPIENTS is empty`), и сообщения не будут доставлены.
