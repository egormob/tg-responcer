...
* `GET /admin/selftest` — выполняет пинг OpenAI и Telegram, всегда отвечает `200`
  и возвращает флаги `openAiOk`/`telegramOk`, коды причин (`openAiReason`,
  `telegramReason`), телеметрию (`openAiLatencyMs`, `telegramStatus`,
  `telegramDescription`) и последний `lastWebhookSnapshot`.
